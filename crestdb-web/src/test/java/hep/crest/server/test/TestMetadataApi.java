...
public class TestMetadataApi {
    ...
    public void testA_runinfoApi() throws Exception {
        ...
        final RunInfoDto dto = DataGenerator.generateRunInfoDto(start, end, new BigDecimal(100L));
        final CrestBaseResponse setdto = new RunInfoSetDto().addResourcesItem(dto).size(1L);
        log.info("Store run info set : {} ", setdto);
        final ResponseEntity<RunInfoSetDto> response = this.testRestTemplate
                .postForEntity("/crestapi/runinfo", setdto, RunInfoSetDto.class);
...

        {
            ...
            ok = mapper.readValue(responseBody, RunInfoSetDto.class);
