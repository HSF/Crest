...
class CMApi(ConditionManagementAbstract):
    ...
    def listTags(self,tagnamepattern):
        ...
        tb = ''
        try:
            self._logger.info('Search tag set using %s' % tagnamepattern)
            searchby='name:{0}'.format(tagnamepattern)
            api_response = api_instance.list_tags(by=searchby)
            self._logger.debug('Retrieved tag set %s' % api_response)
            return api_response
        except ApiException as e:
            print ("Exception when calling TagsApi->find_tag: %s\n" % e)
            print(type(e))
            ...
            print('Got exception from list_tags')
            print(type(inst))
            print(inst)
    ...
    def listAllIovs(self,by,**kwargs):
        ...
        for key, val in params['kwargs'].items():
            ...
            params[key] = val
        del params['kwargs']
        search_params = { 'by' : by, 'page' : 0, 'size' : 100, 'sort' : 'id.since:ASC' }
        if 'sort' in params:
            search_params['sort'] = params['sort']
        if 'page' in params:
        ...
        api_instance = IovsApi(self.api_client)
        try:
            api_response = api_instance.find_all_iovs(by=search_params['by'],
                    page=search_params['page'],size=search_params['size'],sort=search_params['sort'])
            ###print 'Retrieved list of iovs ',api_response
            return api_response
        except ApiException as e:
...
#            print ('CMApi: stored object using %s and returning %s' % (iov_params,iapi_response))
    ...
    def getBlob(self, phash):
        ...
        try:
            #print 'Retrieving payload using hash ',phash
            api_response = api_instance.get_payload(phash,x_crest_payload_format='BLOB',_preload_content=False,_return_http_data_only=True)
            self.__payload['hash'] = phash
            self.__payload['payload'] = api_response
            return api_response
        except Exception as e:
...
class CrestConsole(CMApi):
    ...
    def lstags(self,pattern):
        ...
        self._logger.debug('Retrieved list of tags %s ' % res)
        resources = []
        if res is not None:
    ...
    def lsiovs(self,page,size):
        ...
        if tag is None or len(tag.resources) == 0:
            print ('Cannot list IOVs because tag %s was not found' % self._seltag)
            return InfoList([])
        by = 'tagname:%s' % self._seltag
        res = self.listAllIovs(by,page=page,size=size)
        resources = []
        if res is not None:
            resources = res.resources
        wrapres = []
        ...
        wrapres.append(headeriov)
        for x in resources:
            wrapit = IovDtoWrap(x,tag.resources[0].time_type)
            wrapres.append(wrapit)
        return InfoList(wrapres)
