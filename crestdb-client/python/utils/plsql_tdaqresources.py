...
if __name__ == '__main__':
    try:
        ...
        for stime in iovlist:
            ...
            for row in res:
                rcur=row[0]
                columns = [i[0] for i in rcur.description]
                for irow in rcur:
                    rdic={'FOLDER_NAME' : folder}
                    for i, col in enumerate(columns):
                        #print('%s %s %s' % (i,col,irow[i]))
                        if col in ('IOV_SINCE','IOV_UNTIL','class', 'SYS_INSTIME','CHANNEL_NAME'):
                            rdic[col] = irow[i]
                    print('Insert : %s' % (rdic))
            res.close()
