        ...
        for i, col in enumerate(columns):
           ...
           if select_columns(col):
              ...
              if type(row[i]) == cx_Oracle.LOB :
                 bindata=row[i].read()
                 row_dict[col] = bindata.encode('base64')
...
#    return [dict(zip(columns, row)) for row in cursor]
    ...
    for chan_row in adictarr:
        ...
        condcoolpyld.addRecord(chanid,channame,chaniovsince,chan_row)
...
if __name__ == '__main__':
    try:
        ...
        longopts=['socks','start=','end=','url=','schema=','db=','folder=','tag=','gtag=','jsondump','tracedump']        
...
#             print ('iov list overridden by start time option...: %s'% iovlist)
        ...
        coolpayload = getNodeInfo(cur,schema,db,folder,coolpayload)
...
# Now we parse the resultset
            coolpayload=rows_to_dict_list(rcur, sincetime, coolpayload)
            jblob = coolpayload.pack()
            if iiov <= 1:
                tagname=coolpayload.tag_name
                timetype=coolpayload.iov_base
                ...
                if atag is None:
                    ...
                    cmapi.createTag(tagname,time_type=timetype,object_type=objtype,synchronization='COPY_FROM_COOL',description=description,last_validated_time=lastvt,end_of_validity=eov)
            ...
            cmapi.storeObject(tagname,coolpayload.stime,cdblob,version='1',object_type='COOL_JSON',insertion_time=instime,streamer_info=strinfo)
