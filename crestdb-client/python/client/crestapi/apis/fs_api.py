...
class FsApi(object):
    ...
    def build_tar(self, tagname, snapshot, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.build_tar(tagname, snapshot, callback=callback_function)
        ...
        if kwargs.get('callback'):
            return self.build_tar_with_http_info(tagname, snapshot, **kwargs)
        else:
            (data) = self.build_tar_with_http_info(tagname, snapshot, **kwargs)
            return data

    def build_tar_with_http_info(self, tagname, snapshot, **kwargs):
        """
        Dump a tag into filesystem and retrieve the tar file asynchronously.
        This method allows to request a tar file from the server using a tag specified in input.
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.build_tar_with_http_info(tagname, snapshot, callback=callback_function)
        ...
        if ('tagname' not in params) or (params['tagname'] is None):
            raise ValueError("Missing the required parameter `tagname` when calling `build_tar`")
        # verify the required parameter 'snapshot' is set
        if ('snapshot' not in params) or (params['snapshot'] is None):
            raise ValueError("Missing the required parameter `snapshot` when calling `build_tar`")


        collection_formats = {}

        resource_path = '/fs/tar'.replace('{format}', 'json')
        path_params = {}
