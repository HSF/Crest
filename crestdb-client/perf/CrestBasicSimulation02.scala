...
class CrestBasicSimulation02 extends Simulation {

  val rampUpTimeSecs = 25
  val testTimeSecs   = 300
  val noOfUsers      = 300
  val minWaitMs      = 200 milliseconds
  ...
  val purl = "http://crest-undertow.web.cern.ch/crestapi"
  val snapshot : Long = 1512036032078L;
  val httpConf = http
...
//    val searchiovs =  during(testTimeSecs) {
  ...
  val groupsscn = scenario("Crest select iovs groups").exec(Search.searchiovgroups)
  val iovsscn = scenario("Crest select iovs ranges").exec(Browse.searchiovs)

  // setUp(scn.inject((rampUsers(noOfUsers) over (rampUpTimeSecs seconds))).protocols(httpConf))
  
  setUp(
  groupsscn.inject(rampUsers(noOfUsers) over (rampUpTimeSecs seconds)).throttle(reachRps(300) in (50 seconds), holdFor(3 minute)),
  iovsscn.inject(rampUsers(noOfUsers) over (rampUpTimeSecs seconds)).throttle(reachRps(300) in (50 seconds), holdFor(3 minute))
).protocols(httpConf)
